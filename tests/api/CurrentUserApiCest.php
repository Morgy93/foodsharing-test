...
class CurrentUserApiCest
{
    ...
    private function assertArrayEquals(ApiTester $I, array $expect, array $actual, string $message = '')
    {
        $I->assertEquals(count($expect), count($actual), "{$message}: Count of array element is wrong");
        for ($i = 0; $i < count($expect); ++$i) {
            $I->assertEquals($expect[$i], $actual[$i], "{$message}:  Element {$i} not matchs.");
        }
    }

    private function sortStoresLikeDB(array $items)
    {
    ...
    private function getValuesByKey(array $items, string $key)
    {
        $out = [];
